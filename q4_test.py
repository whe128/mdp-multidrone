...
for i in range(ENV_NUM):
    env_name = f"config_q4_{i+1}.yaml"
    ...
    for r in range(TEST_ROUND):
        ...
        print(f"\r[{env_name}]   with [drone num]: {len(env.get_config().start_positions)}      --test round {r+1}", end=" ")
    ...
    print(f"\r[{env_name}]   with [drone num]: {len(env.get_config().start_positions)}                       ")
